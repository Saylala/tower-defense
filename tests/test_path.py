...
class TestPath(unittest.TestCase):
    def test_simple_field1(self):
        ...
        result = set(game_field.Path(Point(0, 0), Point(0, 0), field).path)
        expected = {Point(0, 0)}
    ...
    def test_simple_field2(self):
        path = game_field.CellType.Path
        field = [[Map(0, 0, path), Map(0, 1, path), Map(0, 2, path)]]
        result = set(game_field.Path(Point(0, 0), Point(0, 1), field).path)
    ...
    def test_simple_field3(self):
        path = game_field.CellType.Path
        field = [[Map(0, 0, path), Map(0, 1, path), Map(0, 2, path)]]
        result = set(game_field.Path(Point(0, 0), Point(0, 2), field).path)
